...
class AlphaID
{
    ...
    public function __construct($salt = 'luffy', $min_hash_length = 4, $alphabet = '')
    {
        ...
        if (is_int(strpos($this->_alphabet, ' '))) {
            throw new InvalidArgumentException('alphabet cannot contain spaces');
        }
    ...
    private function _unhash($input, $alphabet)
    {
        ...
        if ($input_length && $alphabet) {
            ...
            foreach ($input_chars as $i => $char) {
                ...
                if ($this->_math_functions) {
                    $number = $this->_math_functions['str'](
                        $this->_math_functions['add']($number, $pos * pow($alphabet_length, ($input_length - $i - 1)))
                    );
                } else {
